# track the number of people in the room
...
def check_pir():
    global last_triggered, people_count
    ...
    if sensor_1_state and not sensor_2_state:
        ...
        if last_triggered == "sensor_1":
            people_count += 1  # คนเดินเข้า
            print(f"คนเดินเข้า, จำนวนคนในห้อง: {people_count}")
            if client:
                client.publish(sensor_topic, json.dumps({"motion": "in", "people_count": people_count}))
        elif last_triggered == "sensor_2":
            people_count = max(0, people_count - 1)  # คนเดินออก
            print(f"คนเดินออก, จำนวนคนในห้อง: {people_count}")
            if client:
                client.publish(sensor_topic, json.dumps({"motion": "out", "people_count": people_count}))
        last_triggered = None
